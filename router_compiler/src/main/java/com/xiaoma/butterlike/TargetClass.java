...
public class TargetClass {
    ...
    private static final ClassName PARAMETERS_INJECTOR = ClassName.get("com.xiaoma.routerapi",
            "ParametersInject");

    private List<FiledInjecting> filedInjectingList;

    private final TypeTool typeTools;
    private final TypeName targetTypeName;
    private final ClassName injectingClassName;

    TargetClass(TypeTool typeTools, TypeName targetTypeName, ClassName injectingClassName) {
        ...
        for (FiledInjecting filed:filedInjectingList){
            if(filed.getAnnotation().equals(Inject.class)){
                CodeBlock codeBlock = CodeBlock.builder().add("target.$L=", filed.getName())
                        ...
                        .add(buildStatement(typeTools.convertType(filed.getTypeMirror()),true),
                                filed.getParamKey()).build();
